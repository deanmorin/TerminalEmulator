...
DWORD WINAPI ReadThreadProc(LPVOID lpParameter) { //change to handle?
    ...
    pwd = (PWNDDATA) GetWindowLongPtr((HANDLE) lpParameter, 0);
    ...
    while (!bWaitingOnRead) {
        ...
        if (!ReadFile(pwd->hPort, szReadBuf, 1, &dwBytesRead, &overlap)) {
            ...
            //if dwBytesRead?
            //HandleReadBuffer(hWnd, szReadBuf);
            //EchoBuffer(hWnd, szReadBuf, RGB(0,0,0));
        }
